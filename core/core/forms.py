...
class TugasForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'pemberi_tugas': forms.Select(attrs={'class': 'form-select'}),
        ...
        labels = {
            'tanggal_mulai': 'Start Date (Plan)',
            'tenggat_waktu': 'End Date (Plan)',
            'tanggal_mulai_aktual': 'Start Date (Actual)',
            'tanggal_selesai_aktual': 'End Date (Actual)',
        }

    def __init__(self, user, *args, **kwargs):
        super(TugasForm, self).__init__(*args, **kwargs)
        
        # UAT: User Non-Aktif tidak boleh dipilih
        active_users = User.objects.filter(is_active=True)
        self.fields['ditugaskan_ke'].queryset = active_users
        self.fields['pemberi_tugas'].queryset = active_users

        # UAT: Filter Assignment berdasarkan Role
        user_groups = user.groups.all()
        
        if not user.is_superuser:
            # 1. Batasi Proyek & Induk ke Grup User
            ...
            if role == 'MEMBER':
                # UAT: Member hanya bisa assign ke diri sendiri
                self.fields['ditugaskan_ke'].queryset = User.objects.filter(pk=user.pk)
            elif role in ['ADMIN', 'LEADER']:
                # UAT: Admin/Leader assign ke timnya (user yg ada di salah satu grup si Admin)
            ...
            # 3. Pemberi Tugas default current user jika kosong
            if not self.instance.pk:
                self.initial['pemberi_tugas'] = user